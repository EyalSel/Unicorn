...
from unicorn.utils import (gather, is_main_process, postprocess, synchronize,
    ...
    with open(filename, 'w') as f:
        for (frame_id, track_ids, cat_id, H, W, rles) in results:
            for track_id, rle in zip(track_ids, rles):
                if track_id < 0:
                    continue
                line = save_format.format(frame_id=frame_id,
                                          track_id=(2000 + track_id),
                                          cat_id=cat_id,
                                          H=H,
                                          W=W,
                                          rle=rle)
...
class MOTEvaluator:
    ...
    def __init__(self,
        ...
        """
        Args:
            dataloader (Dataloader): evaluate dataloader. img_size (int): image
            size after preprocess. images are resized
                to squares whose shape is (img_size, img_size).
            confthre (float): confidence threshold ranging from 0 to 1, which
                is defined in the config file.
            nmsthre (float): IoU threshold of non-max supression ranging from 0
            to 1.
        """
    ...
    def evaluate(self,
                 ...
                 result_folder=None):
        """
        COCO average precision (AP) Evaluation. Iterate inference on the test
        dataset and the results are evaluated by COCO API.

        NOTE: This function will change training mode to False, please save
        states if needed.
        ...
        Returns:
            ap50_95 (float) : COCO AP of IoU=50:95 ap50 (float) : COCO AP of
            IoU=50 summary (sr): summary info of evaluation.
        """
        ...
        for cur_iter, (imgs, _, info_imgs,
            ...
            with torch.no_grad():
                ...
                if (video_name == 'MOT17-05-FRCNN'
                        or video_name == 'MOT17-06-FRCNN'):
                    self.args.track_buffer = 14
                elif (video_name == 'MOT17-13-FRCNN'
                      or video_name == 'MOT17-14-FRCNN'):
                ...
                # skip the the last iters since batchsize might be not enough
                # for batch inference
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        if len(data_dict) > 0:
            ...
            # TODO: since pycocotools can't process dict in py36, write data to
            # json file.
        ...
        for cur_iter, (imgs, _, info_imgs,
            ...
            with torch.no_grad():
                ...
                if getattr(exp, "use_raft", False):
                    (outputs, locations, dynamic_params, fpn_levels,
                     mask_feats, up_masks) = det_outputs
            ...
            if outputs[0] is not None:
                with torch.no_grad():
                    ...
                    masks = F.interpolate(
                      ...
                      img_w] > self.mask_thres  # (N, height, width)
                    ...
                    embed_cur = model(feat=new_feat_cur,
                                      mode="upsample")  # (1, C, H/8, W/8)
                    pre_dict = copy.deepcopy(cur_dict)
                    track_feat_list = []
                    for i in range(bboxes.size(0)):
                        ...
                        if grid_sample:
                            cx, cy = (x1 + x2) / 2 / s - 0.5, (
                                y1 + y2) / 2 / s - 0.5
                            cx = (torch.clamp(
                                cx, min=0, max=self.img_size[1] // s - 1) /
                                  (self.img_size[1] // s - 1) -
                                  0.5) * 2.0  # range of [-1, 1]
                            cy = (torch.clamp(
                                cy, min=0, max=self.img_size[0] // s - 1) /
                                  (self.img_size[0] // s - 1) -
                                  0.5) * 2.0  # range of [-1, 1]
                            grid = torch.stack([cx, cy],
                                               dim=-1).view(1, 1, 1,
                                                            2)  # (1, 1, 1, 2)
                            track_feat_list.append(
                                F.grid_sample(embed_cur,
                                              grid,
                                              mode='bilinear',
                                              padding_mode='border',
                                              align_corners=False).squeeze())
                        else:
                            cx = torch.round((x1 + x2) / 2 / s).int().clamp(
                                min=0, max=self.img_size[1] // s - 1)
                            cy = torch.round((y1 + y2) / 2 / s).int().clamp(
                                min=0, max=self.img_size[0] // s - 1)
                            track_feat_list.append(embed_cur[0, :, cy, cx])
                    if len(track_feat_list) == 0:
                        track_feats = torch.zeros((0, embed_cur.size(1)))
                    else:
                        track_feats = torch.stack(track_feat_list,
                                                  dim=0)  # (N, C)
                    ...
                    track_inputs = torch.cat(
                        (bboxes, scores),
                        dim=1)  # (N, 5) (x1, y1, x2, y2, conf)
                    # run tracking
                    track_inputs_cpu = track_inputs.cpu()
                    track_feats_cpu = track_feats.cpu()
                    output_bboxes, _, output_ids, indexs = tracker.match(
                        track_inputs_cpu,
                        labels,
                        track_feats_cpu,
                        frame_id,
                    ...
                    masks = masks[inds]
                    output_bboxes_np, output_ids_np = output_bboxes.cpu(
                    ).numpy(), output_ids.cpu().numpy()
                    """post-processing (overlap-free mask prediction)"""
                    if masks.size(0) > 0:
                        masks_new = masks.clone()
                        mask_prev = masks[0].clone()
                        for n in range(1, masks.size(0)):
                            masks_new[n] = torch.logical_and(
                                torch.logical_not(mask_prev), masks[n])
                    ...
                    for i in range(output_bboxes_np.shape[0]):
                        ...
                        if cur_id > -1:
                            ...
                            if w * h > self.args.min_box_area:
                                ...
                                online_ids.append(cur_id +
                                                  1)  # 1-based for MOT
                    results.append(
                        (frame_id, online_ids, 2, img_h, img_w, mask_rle_list))
    ...
    def evaluate_omni(
        ...
        Returns:
            ...
            IoU=50 summary (sr): summary info of evaluation.
        """
        print(">>>>>>>>>>>>>>>", self.confthre, self.img_size, self.nmsthre)
        # TODO half to amp_test
        tensor_type = torch.cuda.HalfTensor if half else torch.cuda.FloatTensor
        model = model.eval()
        if half:
        ...
        for cur_iter, (imgs, _, info_imgs,
            ...
            if (use_external_detections and len(bboxes) > 0) or (outputs[0]
                ...
                with torch.no_grad():
                    ...
                    # vertorized version
                    cx, cy = (bboxes[:, 0] + bboxes[:, 2]) / 2 / s - 0.5, (
                        bboxes[:, 1] + bboxes[:, 3]) / 2 / s - 0.5
                    cx = (
                        torch.clamp(cx, min=0, max=self.img_size[1] // s - 1) /
                        (self.img_size[1] // s - 1) -
                        0.5) * 2.0  # range of [-1, 1]
                    cy = (
                        torch.clamp(cy, min=0, max=self.img_size[0] // s - 1) /
                        (self.img_size[0] // s - 1) -
                        0.5) * 2.0  # range of [-1, 1]
                    ...
                    output_bboxes, _, output_ids = tracker.match(
                        track_inputs_cpu, labels, track_feats_cpu, frame_id)
                    # torch.cuda.synchronize()
                    # match_end = time.time()
                    # remove invalid results
                    valid_inds = (output_ids > -1)
